...
namespace IntegrationTests
{
    ...
    public class ShipmentTest : BaseTest
    {
        ...
        {
            var requestUri = "/api/v2/shipments";
        ...
        {
            var requestUri = "/api/v2/shipments/1";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(result);
        ...
        {
            var requestUri = "/api/v2/shipments/1/items";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(result);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        {
            // create order
            var requestUriOrder = "/api/v2/orders";
            await _client.PostAsJsonAsync(requestUriOrder, _orderAdded);

            // do the test
            var requestUri = "/api/v2/shipments/1/orders";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(result);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        public async Task GetOrdersInShipmentAfterPutting()
        {
            ...
            Xunit.Assert.Single(order);
            Xunit.Assert.Equal(3, order.FirstOrDefault());
            var requestUriOrder = $"/api/v2/orders/{order.FirstOrDefault()}";
        ...
        {
            var requestUri = "/api/v2/orders/3";
