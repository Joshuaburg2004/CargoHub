...
namespace IntegrationTests
{
    ...
    public class Item_typeIntegratieTest : BaseTest
    {
        ...
        private static string _requestUri = "/api/v2/item_types";
        ...
        {
            var requestUri = "/api/v2/item_types";
        ...
        {
            var requestUri = "/api/v2/item_types/1";
        ...
        {
            var requestUri = "/api/v2/item_types/1/items";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(response);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        {
            HttpResponseMessage response = await _client.DeleteAsync("/api/v2/items/P000004");
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        }
    }
}