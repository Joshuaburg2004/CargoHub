...
namespace IntegrationTests
{
    ...
    public class ItemGroupTest : BaseTest
    {
        ...
        {
            var requestUri = "/api/v2/item_groups";
        ...
        {
            var requestUri = "/api/v2/item_groups/1";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(response);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        {
            var requestUri = "/api/v2/item_groups/1/items";
            var response = await _client.GetAsync(requestUri);
            var result = await response.Content.ReadAsStringAsync();
            Xunit.Assert.NotNull(response);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        {
            var requestUri = "/api/v2/item_groups/2";
            var response = await _client.GetAsync(requestUri);
        ...
        {
            var requestUri = "/api/v2/items/P000006";
            var response = await _client.DeleteAsync(requestUri);
            Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        }
    }
