...
public class ItemIntegratieTest : BaseTest
{
    ...
    public static IntegrationTests.models.Item TestItem = new("P000001", "sjQ23408K", "Face-to-face clear-thinking complexity",
    ...
    public static IntegrationTests.models.Item TestPutItem = new("P000001", "item", "item for testing put",
     "must", "6523540947122", "63-OFFTq0T", "oTo304", 0, 0,0,0,0,0,0,"SUP423", "E-86805-uTM");

    public string requestUri = "/api/v1/items";

    public IntegrationTests.models.Inventory TestInventory = new(5, "P000001", "test", "63-OFFTq0T", new List<int>(){3211, 24700, 14123, 19538, 31071, 24701, 11606, 11817}, 40, 40, 40, 40, 40);
    ...
    public async Task GetAllItemsEmpty()
    {
        ...
        List<IntegrationTests.models.Item>? returnedlist = JsonSerializer.Deserialize<List<IntegrationTests.models.Item>>(await response.Content.ReadAsStringAsync());
        Xunit.Assert.IsType<List<IntegrationTests.models.Item>>(returnedlist);
    ...
    public async Task GetItemOne()
    {
        ...
        Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        IntegrationTests.models.Item? ToCompare = JsonSerializer.Deserialize<IntegrationTests.models.Item>(await response.Content.ReadAsStringAsync());
        Xunit.Assert.IsType<IntegrationTests.models.Item>(ToCompare);
    ...
    public async Task GetAllItemsOne()
    {
        ...
        List<IntegrationTests.models.Item>? responselist = JsonSerializer.Deserialize<List<IntegrationTests.models.Item>>(await response.Content.ReadAsStringAsync());
        Xunit.Assert.IsType<List<IntegrationTests.models.Item>>(responselist);
        Xunit.Assert.Single(responselist);

        IntegrationTests.models.Item ToCompare = responselist[0];
    ...
    public async Task GetUpdatedItemTest()
    {
        ...
        IntegrationTests.models.Item? itemtypeafterupdate = JsonSerializer.Deserialize<IntegrationTests.models.Item>(responseContent);

        Xunit.Assert.IsType<IntegrationTests.models.Item>(itemtypeafterupdate);
    ...
    public async Task GetItemInventories()
    {
        ...
        var responseContent = await response.Content.ReadAsStringAsync();
        List<IntegrationTests.models.Inventory>? inventoryresponse = JsonSerializer.Deserialize<List<IntegrationTests.models.Inventory>>(responseContent);
        Xunit.Assert.Equal(HttpStatusCode.OK, response.StatusCode);

        Xunit.Assert.IsType<List<IntegrationTests.models.Inventory>>(inventoryresponse);
        Xunit.Assert.Single(inventoryresponse);
        IntegrationTests.models.Inventory inventoryCompared = inventoryresponse[0];
